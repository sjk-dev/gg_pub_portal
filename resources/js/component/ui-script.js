  ...
  setupFocusOutEvent() {
    document.addEventListener("click", (event) => {
      ...
      if (!clickedInsideTooltip) {
        ...
        const FocusPopover =
          clickedInsideTooltip.querySelector(".tooltip-popover");
...
document.addEventListener("click", function (e) {
  if (e.target.classList.contains("btn-file-add")) {
    const row = document.createElement("div");
    row.classList.add("upload-item");

    row.innerHTML = `
            <input type="file" class="file-input" hidden />
              <button class="krds-btn secondary">파일선택</button>
              <input
                type="text"
                class="krds-input"
                placeholder="선택된 파일 없음"
              />
              <button class="krds-btn tertiary btn-file-del">삭제</button>
          `;

    document.querySelector("#fileArea").appendChild(row);
  }

  if (e.target.classList.contains("btn-file-del")) {
